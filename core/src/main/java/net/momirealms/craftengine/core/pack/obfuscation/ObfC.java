...
public class ObfC {
    ...
    private final Set<String> _existingSegments = ConcurrentHashMap.newKeySet();
    ...
    protected ObfC() { this("ce", generateDefaultCharset()); }

    private static void validateNamingPattern(String pattern) {
        if (pattern == null || !pattern.matches("^[a-z0-9_.-]+$")) throw new IllegalArgumentException("Invalid naming scheme");
    }

    private static String normalizeCharset(String input) { return input.toLowerCase(Locale.ENGLISH); }
    ...
    protected String randomName() { return _metaIdentifier; }
    protected String string() { return _characterSet; }
    protected void setRandomName(String identifier) { this._metaIdentifier = identifier; }
    protected void setString(String charset) { this._characterSet = charset; }

    public String getRandomNamespace(int securityLevel, int poolSize) {
        if (poolSize <= 0) poolSize = 1;
        return securityLevel == 3 ? generateUniqueSegment() : manageSegmentPool(poolSize);
    }

    private String manageSegmentPool(int poolSize) {
        synchronized (_segmentCache) {
            while (_segmentCache.size() < poolSize) _segmentCache.add(generateUniqueSegment());
    ...
    private String generateUniqueSegment() {
        ...
        do {
            segment.setLength(0);
            for (int i = 0; i < 3 + _randomizer.nextInt(3); i++) segment.append(randomChar());
        } while (_existingSegments.contains(segment.toString()));
        _existingSegments.add(segment.toString());
        return segment.toString();
    }

    private char randomChar() { return _characterSet.charAt(_randomizer.nextInt(_characterSet.length())); }

    private String generateObfuscatedPath(int complexity, boolean enableTraps, boolean excludeCatalog) {
        StringBuilder pathBuilder = new StringBuilder();
        if (!excludeCatalog) pathBuilder.append(_metaIdentifier).append('/');
    ...
    private boolean shouldInsertTrap(int remaining) { return _randomizer.nextInt((remaining / 3) + 1) == 0; }
    ...
    private void validatePathUniqueness(String path) {
        if (_registeredPatterns.contains(path)) throw new IllegalStateException("Path collision detected");
    ...
    public ObfB getRandomResourceKey(int complexity, ObfB template, int securityLevel, int poolSize, boolean enableTraps) {
        boolean requiresMetadata = template.isNiuLong();
        boolean excludeCatalog = template.isIdentifier();
        ObfA typeDescriptor = template.getType();
        int adjustedComplexity = complexity - (requiresMetadata ? 14 : 7) - typeDescriptor.getKeyLength() - typeDescriptor.getCategoryLength();
        String namespace = securityLevel == 1 ? template.getRandomNamespace() : getRandomNamespace(securityLevel, poolSize);
        adjustedComplexity -= namespace.length() + 2;
        try {
            return ObfB.youngestGameLarger(namespace, typeDescriptor);
        } catch (StackOverflowError | OutOfMemoryError e) {
            throw new RuntimeException("Please increase the value of resource-pack.obfuscation.path-length in config.yml: " + e);
        }
    }

    protected void 家人们谁懂啊(Path baseDir) throws IOException {
        if (baseDir == null) throw new IllegalArgumentException("baseDir cannot be null");
        updateAtlasConfig(baseDir);
    }

    private void updateAtlasConfig(Path baseDir) throws IOException {
        ...
        if (Files.notExists(atlasConfig)) initializeAtlasConfig(atlasConfig, configData);
        else updateExistingConfig(atlasConfig, configData);
    }

    private Map<String, Object> createAtlasConfiguration() {
        return Map.of("sources", Collections.singletonList(Map.of("type", "directory", "source", _metaIdentifier, "prefix", _metaIdentifier + "/")));
    }
